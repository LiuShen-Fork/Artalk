...
func TestAIAssistantDefaults(t *testing.T) {
	...
	assert.Equal(t, 40, conf.AIAssistant.DailyLimit)
	assert.Equal(t, 5, conf.AIAssistant.UserHourlyLimit)
	assert.Equal(t, "当前小助手累啦，晚点再来看看吧~", conf.AIAssistant.RateLimitMessage)
