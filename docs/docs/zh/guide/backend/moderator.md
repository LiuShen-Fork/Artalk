...
评论拦截使用本地手动关键词进行同步检查。关键词使用英文逗号分隔，同时匹配昵称和评论内容；命中后请求直接返回失败，评论不会写入数据库，也不会发送通知。被拒绝的内容会保留在审核页面中供管理员查看。

```yaml
moderator:
  intercept:
    enabled: true
    keywords: "广告,spam,example.com"
```
